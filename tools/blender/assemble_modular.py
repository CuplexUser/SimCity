...
Infrastructure (power plants, water utilities, transmission pylon) and the glass
curtain-wall towers this script used to generate from Blender primitives now live
as REAL, editable meshes in tools/blender/city_assets.blend, rendered by
render_isos.py — edit them there, not here.
...
# ── box primitive (retained for build_com_tower's fins + mechanical roof) ───────
# The infrastructure (power plants, water tower, pumping station, water pump,
# transmission pylon) this file used to generate from Blender primitives now lives
# as REAL, editable meshes in tools/blender/city_assets.blend, rendered by
# render_isos.py. Only the box helper below is still needed here, by
# build_com_tower (a Kenney-module office tower).
...
# ── Specs ─────────────────────────────────────────────────────────────────────
# Civic buildings (b:3 police, b:4 fire, b:5 hospital, b:6 school, b:7 library)
# used to be assembled here from Kenney modules. They are now REAL, editable meshes
# in tools/blender/city_assets.blend (rendered by render_isos.py) — edit them there.
#
...
# The glass curtain-wall skyline towers (glass_com_slab / setback / twin and the
# glass_res_tower) used to be assembled here from scaled cubes + an emissive
# window-grid shader. They are now REAL, editable meshes in
# tools/blender/city_assets.blend, rendered by render_isos.py. Edit them there.
...
BIG = MID_RES + [
    ...
    # NOTE: the glass curtain-wall skyline towers (z:2:2:6/7/8 commercial, z:1:2:3
    # residential) now live as editable meshes in tools/blender/city_assets.blend
    # and are rendered by render_isos.py — they used to be generated here.
