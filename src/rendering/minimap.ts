import { type World } from '../core/world'
import { type IsoCamera } from './isoCamera'
import { Terrain, Zone } from '../core/tile'

export const MINI_W = 160
export const MINI_H = 128
// Pixel offset from the bottom-right canvas corner to the minimap top-left
export const MINI_MARGIN_R = 10
export const MINI_MARGIN_B = 10

function tileColor(terrain: Terrain, zone: Zone): [number, number, number] {
  if (zone === Zone.Residential) return [34,  139, 34]
  if (zone === Zone.Commercial)  return [30,  63,  168]
  if (zone === Zone.Industrial)  return [184, 134, 11]
  switch (terrain) {
    case Terrain.Grass:  return [90,  138, 60]
    case Terrain.Water:  return [26,  92,  140]
    case Terrain.Dirt:   return [155, 122, 61]
    case Terrain.Forest: return [45,  90,  26]
  }
}

export class Minimap {
  private dirty = true
  private imageData: ImageData

  constructor(private world: World) {
    this.imageData = new ImageData(MINI_W, MINI_H)
  }

  markDirty(): void { this.dirty = true }

  /** Returns the canvas-space rect of the minimap given current canvas size. */
  static bounds(canvasW: number, canvasH: number) {
    return {
      x: canvasW - MINI_W - MINI_MARGIN_R,
      y: canvasH - MINI_H - MINI_MARGIN_B,
      w: MINI_W,
      h: MINI_H,
    }
  }

  /**
   * Draw the minimap into `ctx`, which should be the context of a dedicated
   * MINI_W × MINI_H canvas.  Drawing always starts at (0, 0).
   * `viewportW / viewportH` are the main game canvas dimensions used to
   * project the viewport rectangle onto the minimap.
   */
  draw(
    ctx: CanvasRenderingContext2D,
    camera: IsoCamera,
    viewportW: number,
    viewportH: number,
  ): void {
    const w = MINI_W, h = MINI_H

    // Background
    ctx.fillStyle = 'rgba(0,0,0,0.75)'
    ctx.fillRect(0, 0, w, h)

    if (this.dirty) {
      this.rebake()
      this.dirty = false
    }

    ctx.putImageData(this.imageData, 0, 0)

    // ── Viewport indicator ──────────────────────────────────────────────────
    const scaleX = w / this.world.cols
    const scaleY = h / this.world.rows

    const corners = [
      camera.screenToWorld(0,          0),
      camera.screenToWorld(viewportW,  0),
      camera.screenToWorld(0,          viewportH),
      camera.screenToWorld(viewportW,  viewportH),
    ]

    const colMin = Math.max(0,              Math.min(...corners.map(c => c.col)))
    const colMax = Math.min(this.world.cols, Math.max(...corners.map(c => c.col)))
    const rowMin = Math.max(0,              Math.min(...corners.map(c => c.row)))
    const rowMax = Math.min(this.world.rows, Math.max(...corners.map(c => c.row)))

    const vx = colMin * scaleX
    const vy = rowMin * scaleY
    const vw = Math.max(2, (colMax - colMin) * scaleX)
    const vh = Math.max(2, (rowMax - rowMin) * scaleY)

    ctx.strokeStyle = 'rgba(255,255,255,0.85)'
    ctx.lineWidth   = 1
    ctx.strokeRect(Math.round(vx), Math.round(vy), Math.round(vw), Math.round(vh))

    // Border
    ctx.strokeStyle = 'rgba(255,255,255,0.4)'
    ctx.lineWidth   = 1
    ctx.strokeRect(0, 0, w, h)
  }

  private rebake(): void {
    const { world, imageData } = this
    const scaleX = MINI_W / world.cols
    const scaleY = MINI_H / world.rows
    const d = imageData.data

    for (let row = 0; row < world.rows; row++) {
      for (let col = 0; col < world.cols; col++) {
        const tile = world.get(col, row)
        const [r, g, b] = tileColor(tile.terrain, tile.zone)
        const px = Math.floor(col * scaleX)
        const py = Math.floor(row * scaleY)
        const idx = (py * MINI_W + px) * 4
        d[idx]     = r
        d[idx + 1] = g
        d[idx + 2] = b
        d[idx + 3] = 255
      }
    }
  }
}
