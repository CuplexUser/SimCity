...
// The car silhouette lives in the shared, framework-free `carModel.js` so the
// runtime fallback baker and the atlas pipeline (tools/genCars.mjs) draw the
// *identical* car. Re-export it under the existing names for tileTextures.ts.
import { CAR_DIRS as CAR_DIRS_JS, drawCar as drawCarJS } from './carModel.js'

export type CarDir = 'ne' | 'se' | 'sw' | 'nw' | 'n' | 'e' | 's' | 'w'
export const CAR_DIRS = CAR_DIRS_JS as readonly CarDir[]
export const drawCar = drawCarJS as (
  ctx: Ctx2D, cx: number, cy: number, hw: number, hh: number, dir: CarDir, variant: number,
) => void
